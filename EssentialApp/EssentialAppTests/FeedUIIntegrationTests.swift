...
//
//  Created by Cengizhan Özcan on 14.01.2023.
...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadingMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        loader.completeLoadMore(at: 0) // Burada problem var çözemedim.
//        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user initiated loading completes successfully")
    ...
    func test_loadingFeedIndicator_isVisibleWhileLoadingFeed() {
        ...
        XCTAssertFalse(sut.isShowingLoadingIndicator, "Expected no loading indicator once user initated loading is completed successfully")
        
        sut.simulateUserInitiatedReload()
        XCTAssertTrue(sut.isShowingLoadingIndicator, "Expected loading indicator once user initiates another reload")
        
        loader.completeFeedLoadingWithError(at: 2)
        XCTAssertFalse(sut.isShowingLoadingIndicator, "Expected no loading indicator once user initated loading is completed with error")
    }
    ...
    func test_feedImageView_loadsImageURLWhenVisible() {
        let image0 = makeImage(url: anyURL())
        let image1 = makeImage(url: anyURL())
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for first view once first image loading completes succesfully")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected no loading indicator state change for second view once first image loading completes succesfully")
    ...
    func test_feedImageView_doesNoRenderLoadedImageWhenNotVisibleAnymore() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(with: [makeImage()])
